...
class Stats:
    ...
    def package(self, desired_stats, messages, **kwargs):
        current_messages = list(messages)
        data = {}
        if 'moving_average' in desired_stats:
    ...
    def autoregression(self, messages, order):
        data = [self.decode_data(msg) for msg in messages]
        prices = data['price']
        times = data['time']
        # created the object with proper variables
        smt.arima_model.ARMA(prices, order, dates=times)
